...
const PUBLIC_IMAGES_DIR = path.resolve("./public/project-media");
...
async function processProject(project) {
  ...
  for (const key of assetKeys) {
    ...
    if (typeof url === "string" && url.startsWith("/project-media/")) {
      newProject[key] = url;
      continue;
    }
    ...
    try {
      ...
      newProject[key] =
        `/project-media/${email}/${projectName}/${finalFileName}`;
    } catch (err) {
